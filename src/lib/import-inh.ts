// Núcleo de importación del Excel oficial del INH (repProgramacionNN.xlsx) → base de datos.
...
export type ReunionINH = {
  nroReunion: number | null;
  fecha: string | null;
  carreras: CarreraINH[];
};

export function parsearExcel(buffer: Buffer): ReunionINH {
...
export async function importarReunionDesdeExcel(buffer: Buffer): Promise<{
  nroReunion: number | null;
  fecha: string;
  ...
  return {
    nroReunion: data.nroReunion,
    fecha: data.fecha,
    carrerasCount: data.carreras.length,
    inscritosCount: totalInscritos,
  };
}
